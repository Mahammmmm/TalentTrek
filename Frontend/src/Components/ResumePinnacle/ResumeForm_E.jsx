...
const Questionnaire = () => {
    ...
    if (schoolName !== '' && schoolLocation !== '' && degree !== '' && fieldOfStudy !== '') {
      try {
        ...
        if (response.data.status === 'ok') {
          ...
          navigate("/ResumeFormEdu2_pinnacle", { state: {personalDetailsData,workExperienceData,educationData} });
        } else {
  ...
  const handleBack = (e) => {
    e.preventDefault();
    navigate("/ResumeFormWorkExp2_pinnacle",{state: personalDetailsData})  
